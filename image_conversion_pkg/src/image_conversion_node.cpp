    ...
    void imageCallback(const sensor_msgs::msg::Image::SharedPtr msg) {
        try {
            ...
            if (image_mode_ == 1) { // Grayscale
                // Convert only if the input is a color image
                if (cv_ptr->image.channels() == 3) {
                    RCLCPP_INFO(this->get_logger(), "ImageConversionNode is in Black and White");
                    cv::cvtColor(cv_ptr->image, processed_image, cv::COLOR_BGR2GRAY);
                    cv_ptr->image = processed_image;
                    cv_ptr->encoding = sensor_msgs::image_encodings::MONO8;
                }
            } else {
                // Ensure color image is retained for mode 2
